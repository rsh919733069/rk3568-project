...
HT_API void * sys_os_create_mutex()
{
    ...
    static int index = 0;
    char name[32] = {'\0'};
    sprintf(name, "testmutex%u", (uint32)time(NULL)+index++);
...
{	
    ...
    static int index = 0;
    char name[32] = {'\0'};
    sprintf(name, "testsig%u", (uint32)time(NULL)+index++);
...
int _mem_log_print(char *fmt, va_list argptr)
{
	...
	if (pMemLogFile == NULL)
	{
		...
		{
			log_print(HT_LOG_ERR, "%s, log init sem_init failed[%s]\r\n", __FUNCTION__, strerror(errno));
		}
	}
...
HT_API int mem_log_print(char * fmt,...)
{
	int slen;	
	va_list argptr;
