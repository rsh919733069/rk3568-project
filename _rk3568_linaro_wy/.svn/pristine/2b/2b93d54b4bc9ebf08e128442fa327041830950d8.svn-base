 ...
 * @brief	    Daheng camera video capture.
 * @author 	    WuAng
 * @version 	1.0
 * @date 	    2023/07/26
 *************************************************************************************
...
int capture_thread_main()
{
    printf("\ncapture_thread_main Initializing......\n\n"); 
    ...
    emStatus = GXSetFloat(g_hDevice, GX_FLOAT_ACQUISITION_FRAME_RATE, 60.0);
    GX_VERIFY(emStatus);
    emStatus = GXSetEnum(g_hDevice, GX_ENUM_PIXEL_FORMAT, GX_PIXEL_FORMAT_MONO8);
    GX_VERIFY_EXIT(emStatus);
    emStatus = GXSetEnum(g_hDevice, GX_ENUM_EXPOSURE_AUTO, GX_EXPOSURE_AUTO_CONTINUOUS);
    GX_VERIFY_EXIT(emStatus);
    emStatus = GXSetFloat(g_hDevice, GX_FLOAT_AUTO_EXPOSURE_TIME_MAX, 30000.0);
    GX_VERIFY(emStatus);
...
void *ProcGetImage(void* pParam)
{
    ...
    memset(data_stream->yuv_data,0x80,1920*1080*3/2*20);
    ...
    while(g_bAcquisitionFlag)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(data_stream->yuv_data == NULL)
                {
                    printf("data_stream->yuv_data: %p\n",data_stream->yuv_data);
                }

                uint8_t *yuv_data = data_stream->yuv_data + data_number;
                uint8_t *y_data = (uint8_t *)(pFrameBuffer->pImgBuf);

                for(int h=0;h<1080;h++)
                {
                    for(int w=0;w<1920;w++)
                    {
                        yuv_data[h*1920+w] = y_data[h*2048+w];
                    }
                }
